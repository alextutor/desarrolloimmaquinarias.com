...
class Ai1wmme_Export_Database {
	...
	public static function execute( $params ) {
		...
		if ( isset( $params['options']['sites'] ) ) {
			...
			foreach ( ai1wmme_include_sites( $params ) as $site ) {
				...
				if ( isset( $params['options']['no_spam_comments'] ) ) {
					$mysql->set_table_where_query( ai1wm_table_prefix( $site['BlogID'] ) . 'comments', array( "`comment_approved` != 'spam'" ) )
						->set_table_where_query( ai1wm_table_prefix( $site['BlogID'] ) . 'commentmeta', array( sprintf( "`comment_ID` IN ( SELECT `comment_ID` FROM `%s` WHERE `comment_approved` != 'spam' )", ai1wm_table_prefix( $site['BlogID'] ) . 'comments' ) ) );
				}

				// Exclude post revisions
				if ( isset( $params['options']['no_post_revisions'] ) ) {
					$mysql->set_table_where_query( ai1wm_table_prefix( $site['BlogID'] ) . 'posts', array( "`post_type` != 'revision'" ) );
				}

				// Exclude site options
				$mysql->set_table_where_query( ai1wm_table_prefix( $site['BlogID'] ) . 'options', array( sprintf( "`option_name` NOT IN ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')", AI1WM_ACTIVE_PLUGINS, AI1WM_ACTIVE_TEMPLATE, AI1WM_ACTIVE_STYLESHEET, AI1WM_STATUS, AI1WM_SECRET_KEY, AI1WM_AUTH_USER, AI1WM_AUTH_PASSWORD, AI1WM_BACKUPS_LABELS, AI1WM_SITES_LINKS ) ) );
			...
			// Include users
			if ( $users ) {
				$mysql->set_table_where_query( ai1wm_table_prefix() . 'users', array( sprintf( "`ID` IN ( SELECT `user_id` FROM `%s` WHERE `meta_key` IN ('%s') )", ai1wm_table_prefix() . 'usermeta', implode( "', '", $users ) ) ) );
			}
			...
			// Exclude user meta
			if ( $usermeta ) {
				$mysql->set_table_where_query( ai1wm_table_prefix() . 'usermeta', $usermeta );
			}
		...
		// Exclude site meta
		$mysql->set_table_where_query( ai1wm_table_prefix() . 'sitemeta', array( sprintf( "`meta_key` NOT IN ('%s')", AI1WM_ACTIVE_SITEWIDE_PLUGINS ) ) );
