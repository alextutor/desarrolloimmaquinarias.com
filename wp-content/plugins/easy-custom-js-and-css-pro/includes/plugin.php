...
class EasyCustomJsAndCss {
	...
	function validate_filter_rule($filterRule) {
		...
		switch($field) {
			case 'post': return $this->validate_filter_rule_post__premium_only($filterRule); break;
			case 'page': return $this->validate_filter_rule_page__premium_only($filterRule); break;
			case 'page_template': return $this->validate_filter_rule_page_template__premium_only($filterRule); break;
			case 'device': return $this->validate_filter_rule_device__premium_only($filterRule); break;
			case 'os': return $this->validate_filter_rule_os__premium_only($filterRule); break;
			case 'browser': return $this->validate_filter_rule_browser__premium_only($filterRule); break;
			case 'logged_in': return $this->validate_filter_rule_logged_in__premium_only($filterRule); break;
			case 'user': return $this->validate_filter_rule_user__premium_only($filterRule); break;
			case 'role': return $this->validate_filter_rule_role__premium_only($filterRule); break;
			case 'date': return $this->validate_filter_rule_date__premium_only($filterRule); break;
			case 'time': return $this->validate_filter_rule_time__premium_only($filterRule); break;
			case 'minutes': return $this->validate_filter_rule_minutes__premium_only($filterRule); break;
			case 'hours': return $this->validate_filter_rule_hours__premium_only($filterRule); break;
			case 'day': return $this->validate_filter_rule_day__premium_only($filterRule); break;
			case 'day_of_week': return $this->validate_filter_rule_day_of_week__premium_only($filterRule); break;
			case 'month': return $this->validate_filter_rule_month__premium_only($filterRule); break;
			case 'year': return $this->validate_filter_rule_year__premium_only($filterRule); break;
		}
			...
			if(!$error && wp_is_writable(EASYJC_PLUGIN_UPLOAD_DIR)) {
				...
				if($config->type == 'css') {
					if($config->options->preprocessor == 'less') {
						require_once( plugin_dir_path( dirname(__FILE__) ) . 'includes/lib/lessphp/lessc.inc.php' );
						
						$less = new lessc;
						try {
							$file_data = $less->compile($config->data);
						} catch (exception $ex) {
							$error = true;
							$data['msg'] = __('Can\'t compile LESS data.<br>Message: ', EASYJC_PLUGIN_NAME);
							$data['msg'] = $data['msg'] . $ex->getMessage();
						}
					} else if($config->options->preprocessor == 'scss') {
						require_once( plugin_dir_path( dirname(__FILE__) ) . 'includes/lib/scssphp/scss.inc.php' );
						...
						try {
							...
							$error = true;
							$data['msg'] = __('Can\'t compile SCSS data.<br>Message: ', EASYJC_PLUGIN_NAME);
							$data['msg'] = $data['msg'] . $ex->getMessage();
						}
					}
				}
