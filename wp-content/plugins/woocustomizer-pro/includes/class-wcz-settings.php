	...
	private function settings_fields() {
		...
		if ( ! wcz_fs()->can_use_premium_code() ) {
			...
			$settings['wcz_catalogue'] = array(
				...
				'fields'      => array(
					array(
						...
						'description' => __( 'Upgrade from only <b>$29</b> to unlock these extra Pro features', 'woocustomizer' ),
						'type'        => 'promo',
						'default'     => '',
			...
			$settings['wcz_handheld_footer'] = array(
				...
				'fields'      => array(
					array(
						...
						'description' => 'Upgrade from only <b>$29</b> to unlock these extra Pro features',
						'type'        => 'promo',
                        'default'     => '',
			...
			$settings['wcz_product_badges'] = array(
				...
				'fields'      => array(
					array(
						'id'          => 'product_badges_promo',
						'label'       => __( 'WooCommerce Product Badges', 'woocustomizer' ),
						'description' => 'The price will increase once WooCommerce Custom Product Badges is released.<br /><a href="' . admin_url( 'admin.php?page=wcz_settings-pricing' ) . '">Purchase StoreCustomizer Pro</a> now and <b>pay only $29</b>.',
						'type'        => 'promo',
                        'default'     => '',
		...
		if ( wcz_fs()->can_use_premium_code() ) {
			...
			$wcz_menus = get_registered_nav_menus(); // wp_get_nav_menus()
			$wcz_arr = array( 'none' => 'None' );

			foreach ( $wcz_menus as $location => $description ) {
				$wcz_arr[$location] = $description;
			}
			// foreach ( $wcz_menus as $wcz_menu ) {
			// 	$wcz_arr[$wcz_menu->slug] = $wcz_menu->name;
			// }
			...
			// $settings['wcz_product_badges'] = array(
			// 	'title'       => __( 'Product Badges', 'woocustomizer' ),
			// 	'description' => __( 'Do you want fancy badges for your WooCommerce Products? Turn on StoreCustomizer Product Badges and start creating your own badges under Products -> Product Badges.', 'woocustomizer' ),
			// 	'fields'      => array(
			// 		array(
		...
		if ( is_array( $this->settings ) && 1 < count( $this->settings ) ) {
			...
			foreach ( $this->settings as $section => $data ) {
				...
				if ( 'produ' == strtolower( $tabnew ) ) // ADJUST FOR NEW TABS
				 	$tabisnew = '<span class="isnew">COMING SOON</span>';
			...
			if ( ! wcz_fs()->can_use_premium_code() ) {
				// Last Upgrade tab.
				$html .= '<a href="' . esc_url( admin_url( 'admin.php?page=wcz_settings-pricing' ) ) . '" class="' . esc_attr( $class ) . ' wcz-upgrade-tab">' . esc_html( __( 'Upgrade', 'woocustomizer' ) ) . '</a>' . "\n";
			}
