...
function wcz_admin_body_class( $admin_classes ) {
    ...
    if ( wcz_fs()->can_use_premium_code__premium_only() ) {
        $admin_classes .= ' wcz-pro';
    } else {
        $admin_classes .= ' wcz-free';
    }

    if ( current_user_can( 'manage_options' ) && !get_user_meta( $wcz_user_id, 'wcz_pricechange_notice_dismiss' ) ) {
...
function wcz_wc_texts() {
	...
	if ( is_woocommerce() || is_cart() ) {
        ...
        if ( get_option( 'wcz-edit-stockstatus', woocustomizer_library_get_default( 'wcz-edit-stockstatus' ) ) ) {
            add_filter( 'woocommerce_get_availability', 'wcz_stock_availability_text', 99, 2 );
        }
	...
	if ( get_option( 'wcz-set-cart-increment-vals', woocustomizer_library_get_default( 'wcz-set-cart-increment-vals' ) ) ) {
		add_filter( 'woocommerce_quantity_input_args', 'wcz_set_product_single_min_max_values', 10, 2 );
    }
...
if ( wcz_fs()->can_use_premium_code__premium_only() ) {
	...
	// Add - button to single Product input
	function wcz_add_product_single_minus_btn() {
		echo '<div class="wcz-product-pm-btns"><div class="wcz-pm-btns"><button type="button" class="minus" >-</button>';
	}
	// Add + button to single Product input
	function wcz_add_product_single_plus_btn() {
		echo '<button type="button" class="plus" >+</button></div></div>';
	}
	// Add pretty + & - buttons to single Product input
	function wcz_edit_product_single_plus_minus() {
		// IF not single product page then return
		if ( !is_product() )
		    return;
		?>
...
function wcz_set_product_single_min_max_values( $args, $product ) {
	...
	if ( !is_cart() ) {
		$args['min_value'] = esc_attr( $arg_min ); // Min quantity
		$args['max_value'] = esc_attr( $arg_max ); // Max quantity (default -1)
		// $args['input_value'] = 4; // Start at
		$args['step'] = $arg_step; // Increment by
	} else {
		// 'min_value' is already 0
		$args['min_value'] = esc_attr( $arg_min );
		$args['max_value'] = esc_attr( $arg_max ); // Max quantity
		$args['step'] = $arg_step;
	}
	return $args;
}
...
function wcz_add_backtoshop_cart_button() {
    ...
    $wcz_btslink = $wcz_btsurl ? $wcz_btsurl : get_permalink( wc_get_page_id( 'shop' ) );
...
if ( get_option( 'wcz-checkout-remove-firstname', woocustomizer_library_get_default( 'wcz-checkout-remove-firstname' ) ) ||
get_option( 'wcz-checkout-remove-firstname', woocustomizer_library_get_default( 'wcz-checkout-remove-firstname' ) ) ||
get_option( 'wcz-checkout-remove-lastname', woocustomizer_library_get_default( 'wcz-checkout-remove-lastname' ) ) ||
get_option( 'wcz-checkout-remove-company', woocustomizer_library_get_default( 'wcz-checkout-remove-company' ) ) ||
get_option( 'wcz-checkout-remove-address', woocustomizer_library_get_default( 'wcz-checkout-remove-address' ) ) ||
get_option( 'wcz-checkout-remove-citystate', woocustomizer_library_get_default( 'wcz-checkout-remove-citystate' ) ) ||
get_option( 'wcz-checkout-remove-phone', woocustomizer_library_get_default( 'wcz-checkout-remove-phone' ) ) ) {
...
function wcz_add_notice_banner_wc() {
    ...
    <div class="wcz-banner-notice <?php echo get_option( 'wcz-wc-notice-design', woocustomizer_library_get_default( 'wcz-wc-notice-design' ) ) ? sanitize_html_class( $wcz_notice_style ) : sanitize_html_class( 'wcz-notice-one' ); ?>">
        <h4><span><?php esc_html_e( $wcz_notice_title ); ?></span></h4>
        <p><?php esc_html_e( $wcz_notice_text ); ?></p>
    </div><?php
}
...
function wcz_add_admin_stats_btn() {
    ...
    if ( current_user_can('manage_options') ) { ?>
        <button class="wcz-adminstats-btn" title="<?php esc_attr_e( 'View Product Statistics', 'woocustomizer' ); ?>" data-productid="<?php echo esc_attr( $product->get_id() ); ?>"></button>
    <?php
    }
}
// Add Footer Modal
...
function wcz_admin_get_product_stats() {
	...
	ob_start(); ?>
		<div class="wcz-adminstats-modal-inner">
			<h4><span><?php esc_html_e( $product->get_name() ); ?></span><span><?php esc_html_e( $product->get_type() ); ?> <?php esc_html_e( 'product', 'woocustomizer' ); ?></span></h4>

			<div class="wcz-adminstats-block">
				<div class="wcz-adminstats-title">
					<?php esc_html_e( 'Total Sales', 'woocustomizer' ); ?>
				</div>
				<div class="wcz-adminstats-stat">
					<?php esc_attr_e( $product->get_total_sales() ); ?>
				</div>
			</div>
			<?php
			// global $product;
			...
			if ( !empty( $orders ) ) {
				...
				foreach ( $orders as $order ) {
					...
					if ( $items ) {
						foreach( $items as $item ) {
							...
							if ( $product_id == $product_item_id ) {
								if ( $loop == 0 ) echo '<h5>' . esc_html__( 'Recent Sales', 'woocustomizer' ) . '</h5>'; ?>
								<div class="wcz-adminstats-block">
									<div class="wcz-adminstats-date">
										<?php echo $order->get_date_completed()->format ( 'Y-m-d' ); ?>
									</div>
									<div class="wcz-adminstats-title">
										<?php
									...
									<div class="wcz-adminstats-stat">
										<a href="<?php echo esc_url( $order->get_edit_order_url() ); ?>"><?php esc_html_e( 'View Order', 'woocustomizer' ); ?></a>
									</div>
								</div>
								<?php
								$loop++;
					...
					if ( $loop == $product_limit ) break;
				}
			} ?>

			<div class="wcz-adminstats-edit">
				<a href="<?php echo esc_url( get_edit_post_link( $product_id ) ); ?>"><?php esc_html_e( 'Edit Product', 'woocustomizer' ); ?></a>
			</div>
		</div>
	<?php
    echo ob_get_clean();
...
function wcz_wc_extras() {
    ...
    if ( 'wcz-account-orders-edit' == get_option( 'wcz-account-orders-tab', woocustomizer_library_get_default( 'wcz-account-orders-tab' ) ) ) {
        add_filter( 'woocommerce_endpoint_orders_title', 'wcz_account_endpoint_title', 10, 2 );
    }
    if ( 'wcz-account-downloads-edit' == get_option( 'wcz-account-downloads-tab', woocustomizer_library_get_default( 'wcz-account-downloads-tab' ) ) ) {
        add_filter( 'woocommerce_endpoint_downloads_title', 'wcz_account_endpoint_title', 10, 2 );
    }
    if ( 'wcz-account-address-edit' == get_option( 'wcz-account-address-tab', woocustomizer_library_get_default( 'wcz-account-address-tab' ) ) ) {
        add_filter( 'woocommerce_endpoint_edit-address_title', 'wcz_account_endpoint_title', 10, 2 );
    }
    if ( 'wcz-account-details-edit' == get_option( 'wcz-account-details-tab', woocustomizer_library_get_default( 'wcz-account-details-tab' ) ) ) {
        add_filter( 'woocommerce_endpoint_edit-account_title', 'wcz_account_endpoint_title', 10, 2 );
    }
    // Rename Account Page Titles/Endpoints
    
    if ( get_option( 'wcz-add-price-prefix', woocustomizer_library_get_default( 'wcz-add-price-prefix' ) ) ||
         get_option( 'wcz-add-price-suffix', woocustomizer_library_get_default( 'wcz-add-price-suffix' ) ) ||
    ...
    if ( is_cart() && get_option( 'wcz-cart-disable-cart-quantity', woocustomizer_library_get_default( 'wcz-cart-disable-cart-quantity' ) ) ) {
        add_filter( 'woocommerce_cart_item_quantity', 'wcz_disable_cart_item_quantity', 10, 3 );
    }
...
function wcz_checkout_custom_secureimg() { ?>
	<div class="wcz-checkout-secureimg <?php echo get_option( 'wcz-checkout-img-center', woocustomizer_library_get_default( 'wcz-checkout-img-center' ) ) ? sanitize_html_class( 'wcz-checkout-centerimg' ) : ''; ?>">
		<?php if ( get_option( 'wcz-checkout-img', woocustomizer_library_get_default( 'wcz-checkout-img' ) ) ) : ?>
			<img src="<?php echo esc_url( get_option( 'wcz-checkout-img', woocustomizer_library_get_default( 'wcz-checkout-img' ) ) ); ?>" />
		<?php else : ?>
			<?php esc_html_e( 'Please Upload an Image', 'woocustomizer' ); ?>
		<?php endif; ?>
	</div>
<?php
}
function wcz_checkout_text_below_placeorder() { ?>
	<div class="wcz-checkout-potxt">
		<small>
			<?php echo esc_html( get_option( 'wcz-checkout-po-txt', woocustomizer_library_get_default( 'wcz-checkout-po-txt' ) ) ); ?>
		</small>
	</div>
<?php
}
function wcz_product_amount_sold() {
	...
	if ( $wcz_amntsold ) echo '<div class="wcz-stock-sold">' . str_ireplace( '[no]', $wcz_amntsold, $wcz_stocktxt ) . '</div>';
}
...
if ( ! function_exists( 'wcz_remove_account_links' ) ) {
	...
	function wcz_remove_account_links( $menu_links ) {
		...
		if ( 'wcz-account-orders-edit' == get_option( 'wcz-account-orders-tab', woocustomizer_library_get_default( 'wcz-account-orders-tab' ) ) ||
			'wcz-account-downloads-edit' == get_option( 'wcz-account-downloads-tab', woocustomizer_library_get_default( 'wcz-account-downloads-tab' ) ) ||
			'wcz-account-address-edit' == get_option( 'wcz-account-address-tab', woocustomizer_library_get_default( 'wcz-account-address-tab' ) ) ||
			'wcz-account-details-edit' == get_option( 'wcz-account-details-tab', woocustomizer_library_get_default( 'wcz-account-details-tab' ) ) ) {
		...
		if ( wcz_fs()->can_use_premium_code__premium_only() ) {
			...
			// Custom Endpoint One Tab Title
			if ( get_option( 'wcz-accendpoint-one', woocustomizer_library_get_default( 'wcz-accendpoint-one' ) ) ) {
				$menu_links['wcz-accendpoint-one'] = esc_html( get_option( 'wcz-account-tab-one-tab', woocustomizer_library_get_default( 'wcz-account-tab-one-tab' ) ) );
			}
			// Custom Endpoint Two Tab Title
			if ( get_option( 'wcz-accendpoint-two', woocustomizer_library_get_default( 'wcz-accendpoint-two' ) ) ) {
				$menu_links['wcz-accendpoint-two'] = esc_html( get_option( 'wcz-account-tab-two-tab', woocustomizer_library_get_default( 'wcz-account-tab-two-tab' ) ) );
			}
		...
		if ( wcz_fs()->can_use_premium_code__premium_only() ) {
			...
			// Custom Endpoint Three Tab Title
			if ( get_option( 'wcz-accendpoint-three', woocustomizer_library_get_default( 'wcz-accendpoint-three' ) ) ) {
				$menu_links['wcz-accendpoint-three'] = esc_html( get_option( 'wcz-account-tab-three-tab', woocustomizer_library_get_default( 'wcz-account-tab-three-tab' ) ) );
			}
			// Custom Endpoint Four Tab Title
			if ( get_option( 'wcz-accendpoint-four', woocustomizer_library_get_default( 'wcz-accendpoint-four' ) ) ) {
				$menu_links['wcz-accendpoint-four'] = esc_html( get_option( 'wcz-account-tab-four-tab', woocustomizer_library_get_default( 'wcz-account-tab-four-tab' ) ) );
			}
...
function wcz_account_endpoint_title( $title, $id ) {
	if ( is_wc_endpoint_url( 'orders' ) && !is_admin() && in_the_loop() && is_account_page() ) { // add your endpoint urls
		$title = esc_html( get_option( 'wcz-account-tab-orders-title', woocustomizer_library_get_default( 'wcz-account-tab-orders-title' ) ) ); // change your entry-title
	}
        ...
        if ( $args->theme_location == get_option( 'wcz-login-logout-menu', woocustomizer_library_get_default( 'wcz-login-logout-menu' ) ) ) {
			...
			ob_start();
				if ( is_user_logged_in() ) : ?>
					<a href="<?php echo esc_url( wp_logout_url( $wcz_logout_redirecturl ) ); ?>"><?php echo esc_html( $wcz_logout_txt ); ?></a>
				<?php
				else : ?>
					<a href="<?php echo esc_url( $wcz_login_url ); ?>"><?php echo esc_html( $wcz_login_txt ); ?></a>
				<?php
				endif;
            $items .= ob_get_clean();
            $items .= '</li>';
...
add_filter( 'wp_nav_menu_items', 'wcz_add_menu_login_logout', 10, 2 );
...
if ( wcz_fs()->can_use_premium_code__premium_only() ) {
	 ...
	 */
	function wcz_custom_endpoints() {
		if ( get_option( 'wcz-accendpoint-one', woocustomizer_library_get_default( 'wcz-accendpoint-one' ) ) ) {
            add_rewrite_endpoint( 'wcz-accendpoint-one', EP_ROOT | EP_PAGES );
		}
		if ( get_option( 'wcz-accendpoint-two', woocustomizer_library_get_default( 'wcz-accendpoint-two' ) ) ) {
			add_rewrite_endpoint( 'wcz-accendpoint-two', EP_ROOT | EP_PAGES );
		}
		if ( get_option( 'wcz-accendpoint-three', woocustomizer_library_get_default( 'wcz-accendpoint-three' ) ) ) {
			add_rewrite_endpoint( 'wcz-accendpoint-three', EP_ROOT | EP_PAGES );
		}
		if ( get_option( 'wcz-accendpoint-four', woocustomizer_library_get_default( 'wcz-accendpoint-four' ) ) ) {
			add_rewrite_endpoint( 'wcz-accendpoint-four', EP_ROOT | EP_PAGES );
		}
	}
    add_action( 'init', 'wcz_custom_endpoints' );

	/**
	 * Add Endpoint One to query_vars.
	 */
	function wcz_custom_query_vars( $vars ) {
		if ( get_option( 'wcz-accendpoint-one', woocustomizer_library_get_default( 'wcz-accendpoint-one' ) ) ) {
			$vars[] = 'wcz-accendpoint-one';
		}
		if ( get_option( 'wcz-accendpoint-two', woocustomizer_library_get_default( 'wcz-accendpoint-two' ) ) ) {
			$vars[] = 'wcz-accendpoint-two';
		}
		if ( get_option( 'wcz-accendpoint-three', woocustomizer_library_get_default( 'wcz-accendpoint-three' ) ) ) {
			$vars[] = 'wcz-accendpoint-three';
		}
		if ( get_option( 'wcz-accendpoint-four', woocustomizer_library_get_default( 'wcz-accendpoint-four' ) ) ) {
			$vars[] = 'wcz-accendpoint-four';
		}
		return $vars;
	}
    add_filter( 'query_vars', 'wcz_custom_query_vars', 0 );
    
	/**
	 * Endpoint HTML content.
	 */
	if ( get_option( 'wcz-accendpoint-one', woocustomizer_library_get_default( 'wcz-accendpoint-one' ) ) ) {
        // Custom Tab One Content
		function wcz_custom_endpoint_content_one() {
			$wcz_pageid_one = get_option( 'wcz-account-tab-one-pageid', woocustomizer_library_get_default( 'wcz-account-tab-one-pageid' ) );
			if ( $wcz_pageid_one ) {
				$wcz_page = get_page( $wcz_pageid_one );
        ...
        add_action( 'woocommerce_account_wcz-accendpoint-one_endpoint', 'wcz_custom_endpoint_content_one' );
        // Custom Tab One Title
        function wcz_custom_endpoint_title_one( $title, $endpoint ) {
            global $wp_query;
            $is_endpoint = isset( $wp_query->query_vars[ 'wcz-accendpoint-one' ] );
            // var_dump( $is_endpoint );
			if ( $is_endpoint && !is_admin() && in_the_loop() && is_account_page() ) {
				$title = esc_html( get_option( 'wcz-account-tab-one-title', woocustomizer_library_get_default( 'wcz-account-tab-one-title' ) ) );
            }
            return $title;
        }
        add_filter( 'the_title', 'wcz_custom_endpoint_title_one', 10, 2 );
	}
	if ( get_option( 'wcz-accendpoint-two', woocustomizer_library_get_default( 'wcz-accendpoint-two' ) ) ) {
        // Custom Tab Two Content
		function wcz_custom_endpoint_content_two() {
			$wcz_pageid_two = get_option( 'wcz-account-tab-two-pageid', woocustomizer_library_get_default( 'wcz-account-tab-two-pageid' ) );
			if ( $wcz_pageid_two ) {
				$wcz_page = get_page( $wcz_pageid_two );
				echo apply_filters( 'the_content', $wcz_page->post_content );
			} else {
				esc_html_e( 'Please select the page you\'d like to display here.', 'woocustomizer' );
			}
		}
        add_action( 'woocommerce_account_wcz-accendpoint-two_endpoint', 'wcz_custom_endpoint_content_two' );
        // Custom Tab Two Title
        function wcz_custom_endpoint_title_two( $title, $endpoint ) {
            global $wp_query;
            $is_endpoint = isset( $wp_query->query_vars[ 'wcz-accendpoint-two' ] );
            // var_dump( $is_endpoint );
			if ( $is_endpoint && !is_admin() && in_the_loop() && is_account_page() ) {
				$title = esc_html( get_option( 'wcz-account-tab-two-title', woocustomizer_library_get_default( 'wcz-account-tab-two-title' ) ) );
            }
            return $title;
        }
        add_filter( 'the_title', 'wcz_custom_endpoint_title_two', 10, 2 );
	}
	if ( get_option( 'wcz-accendpoint-three', woocustomizer_library_get_default( 'wcz-accendpoint-three' ) ) ) {
        // Custom Tab Three Content
		function wcz_custom_endpoint_content_three( $wcz_pageid_three ) {
			$wcz_pageid_three = get_option( 'wcz-account-tab-three-pageid', woocustomizer_library_get_default( 'wcz-account-tab-three-pageid' ) );
			if ( $wcz_pageid_three ) {
				$wcz_page = get_page( $wcz_pageid_three );
				echo apply_filters( 'the_content', $wcz_page->post_content );
			} else {
				esc_html_e( 'Please select the page you\'d like to display here.', 'woocustomizer' );
			}
		}
        add_action( 'woocommerce_account_wcz-accendpoint-three_endpoint', 'wcz_custom_endpoint_content_three' );
        // Custom Tab Three Title
        function wcz_custom_endpoint_title_three( $title, $endpoint ) {
            global $wp_query;
            $is_endpoint = isset( $wp_query->query_vars[ 'wcz-accendpoint-three' ] );
            // var_dump( $is_endpoint );
			if ( $is_endpoint && !is_admin() && in_the_loop() && is_account_page() ) {
				$title = esc_html( get_option( 'wcz-account-tab-three-title', woocustomizer_library_get_default( 'wcz-account-tab-three-title' ) ) );
            }
            return $title;
        }
        add_filter( 'the_title', 'wcz_custom_endpoint_title_three', 10, 2 );
	}
	if ( get_option( 'wcz-accendpoint-four', woocustomizer_library_get_default( 'wcz-accendpoint-four' ) ) ) {
        // Custom Tab Four Content
		$wcz_pageid_four = get_option( 'wcz-account-tab-four-pageid', woocustomizer_library_get_default( 'wcz-account-tab-four-pageid' ) );
		function wcz_custom_endpoint_content_four() {
			if ( $wcz_pageid_four ) {
				$wcz_page = get_page( $wcz_pageid_four );
				echo apply_filters( 'the_content', $wcz_page->post_content );
			} else {
				esc_html_e( 'Please select the page you\'d like to display here.', 'woocustomizer' );
			}
		}
        add_action( 'woocommerce_account_wcz-accendpoint-four_endpoint', 'wcz_custom_endpoint_content_four' );
        // Custom Tab Four Title
        function wcz_custom_endpoint_title_four( $title, $endpoint ) {
            global $wp_query;
            $is_endpoint = isset( $wp_query->query_vars[ 'wcz-accendpoint-four' ] );
            // var_dump( $is_endpoint );
			if ( $is_endpoint && !is_admin() && in_the_loop() && is_account_page() ) {
				$title = esc_html( get_option( 'wcz-account-tab-four-title', woocustomizer_library_get_default( 'wcz-account-tab-four-title' ) ) );
            }
            return $title;
        }
        add_filter( 'the_title', 'wcz_custom_endpoint_title_four', 10, 2 );
	}

	// Add Page State to WP admin Pages List
	function wcz_content_page_states( $states, $post ) {
		$wcz_pageid_one = get_option( 'wcz-account-tab-one-pageid', woocustomizer_library_get_default( 'wcz-account-tab-one-pageid' ) );
		if ( 'page' === get_post_type( $post->ID ) && intval( $wcz_pageid_one ) === intval( $post->ID ) ) {
			$states[] = __( 'Account Tab 1', 'woocustomizer' );
		}
		$wcz_pageid_two = get_option( 'wcz-account-tab-two-pageid', woocustomizer_library_get_default( 'wcz-account-tab-two-pageid' ) );
		if ( 'page' === get_post_type( $post->ID ) && intval( $wcz_pageid_two ) === intval( $post->ID ) ) {
			$states[] = __( 'Account Tab 2', 'woocustomizer' );
		}
		$wcz_pageid_three = get_option( 'wcz-account-tab-three-pageid', woocustomizer_library_get_default( 'wcz-account-tab-three-pageid' ) );
		if ( 'page' === get_post_type( $post->ID ) && intval( $wcz_pageid_three ) === intval( $post->ID ) ) {
			$states[] = __( 'Account Tab 3', 'woocustomizer' );
		}
		$wcz_pageid_four = get_option( 'wcz-account-tab-four-pageid', woocustomizer_library_get_default( 'wcz-account-tab-four-pageid' ) );
		if ( 'page' === get_post_type( $post->ID ) && intval( $wcz_pageid_four ) === intval( $post->ID ) ) {
			$states[] = __( 'Account Tab 4', 'woocustomizer' );
		}
		return $states;
	}
	add_filter( 'display_post_states', 'wcz_content_page_states', 10, 2 );

	/**
	 * Flush rewrite rules on plugin activation.
	 */
	function wcz_flush_rewrite_rules() {
		// Flush Endpoint One
		add_rewrite_endpoint( 'wcz-accendpoint-one', EP_ROOT | EP_PAGES );
		add_rewrite_endpoint( 'wcz-accendpoint-two', EP_ROOT | EP_PAGES );
		add_rewrite_endpoint( 'wcz-accendpoint-three', EP_ROOT | EP_PAGES );
	...
	register_activation_hook( __FILE__, 'wcz_flush_rewrite_rules' );
	register_deactivation_hook( __FILE__, 'wcz_flush_rewrite_rules' );
	/**
	 * ------------------------------------------------------------------------------------ Add New Account Tabs.
	 */
