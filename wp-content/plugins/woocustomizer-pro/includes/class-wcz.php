...
class WooCustomizer {
	...
	public function __construct( $file = '', $version = WCD_PLUGIN_VERSION ) {
        ...
        if ( ! wcz_fs()->can_use_premium_code__premium_only() ) {
            add_action( 'admin_init', array( $this, 'wcz_pricechange_notice_ignore' ), 0 );
            add_action( 'admin_notices', array( $this, 'wcz_pricechange_notice' ) );
        }
	...
	public function register_post_type( $post_type = '', $plural = '', $single = '', $description = '', $options = array() ) {
		...
		$post_type = new WooCustomizer_Post_Type( $post_type, $plural, $single, $description, $options );
	...
	public function register_taxonomy( $taxonomy = '', $plural = '', $single = '', $post_types = array(), $taxonomy_args = array() ) {
		...
		$taxonomy = new WooCustomizer_Taxonomy( $taxonomy, $plural, $single, $post_types, $taxonomy_args );
	...
	public function enqueue_scripts() {
		if ( get_option( 'wcz-cart-ajax-update', woocustomizer_library_get_default( 'wcz-cart-ajax-update' ) ) ) :
			wp_register_script( $this->_token . '-cart-update', esc_url( $this->assets_url ) . 'js/wcz-cart-update.js', array( 'jquery' ), $this->_version );
	...
	public function customizer_enqueue_styles( $hook = '' ) {
		wp_register_style( $this->_token . '-customizer', esc_url( $this->includes_url ) . 'customizer/customizer-library/css/customizer.css', array(), $this->_version );
	...
	public function customizer_enqueue_scripts( $hook = '' ) {
		...
		wp_register_script( $this->_token . '-customizer', esc_url( $this->includes_url ) . 'customizer/customizer-library/js/customizer-custom.js', array( 'jquery', 'customize-controls' ), $this->_version, true );
		...
		// Customizer Tour JS
		wp_register_script( $this->_token . '-customizer-tour', esc_url( $this->includes_url ) . 'customizer/customizer-library/js/customizer-tour.js', array( 'jquery', 'customize-controls' ), $this->_version, true );
	...
	public function wcz_tour_steps() {
		...
		if ( wcz_fs()->can_use_premium_code__premium_only() ) {
			...
			'on' == get_option( 'wcz_set_enable_menu_cart', woocustomizer_library_get_default( 'wcz_set_enable_menu_cart' ) ) ||
			'on' == get_option( 'wcz_set_enable_product_quickview', woocustomizer_library_get_default( 'wcz_set_enable_product_quickview' ) ) ||
			'on' == get_option( 'wcz_set_enable_ajax_search', woocustomizer_library_get_default( 'wcz_set_enable_ajax_search' ) ) ) {
				
				$steps[] = array(
					'title'    => __( 'Edit your StoreCustomizer Pro Settings!', 'woocustomizer' ),
					'message'  => __( 'These panels appear as you turn on the pro features you want... The settings for Catalogue Mode, Product Quick View, Menu Cart and / or Ajax Search are all found within these panels. - Start Customizing!', 'woocustomizer' ),
					'top'      => esc_attr( '375' )
				);
	...
	public function wcz_feedback_notice() {
		...
		if ( current_user_can( 'manage_options' ) && !get_user_meta( $wcz_user_id, 'wcz_feedback_notice_dismiss' ) ) : ?>
			<div class="notice notice-info wcz-admin-notice">
                <h4>
                    <?php esc_html_e( 'Thank you for trying out StoreCustomizer !', 'woocustomizer' ); ?>
                    <span>
                        <?php
                        /* translators: 1: 'StoreCustomizer'. */
                        printf( esc_html__( '(Previously known as "%1$s")', 'woocustomizer' ), wp_kses( '<b>WooCustomizer</b>', array( 'b' => array() ) ) ); ?>
                    </span>
                </h4>

				<div class="wcz-notice-columns">
					<div class="wcz-notice-col">
						<h5><?php esc_html_e( 'Customize your WooCommerce store.', 'woocustomizer' ); ?></h5>
						<div class="wcz-notice-cont">
							<p>
                                <?php
                                /* translators: 1: 'WordPress Customizer'. */
                                printf( esc_html__( 'StoreCustomizer settings are built into the %1$s -> StoreCustomizer. You can navigate your way around and edit all the settings live.', 'woocustomizer' ), wp_kses( '<a href="' . admin_url( 'customize.php' ) . '" class="wcz-admin-notice-a">' . __( 'WordPress Customizer', 'woocustomizer' ) . '</a>', array( 'a' => array( 'href' => array(), 'class' => array() ) ) ) ); ?>
                            </p>
                            <p><?php esc_html_e( 'For more on StoreCustomizer, please see:', 'woocustomizer' ); ?></p>
						</div>
						<a href="<?php echo admin_url( 'admin.php?page=wcz_settings' ); ?>" class="wcz-notice-btn">
							<?php esc_html_e( 'StoreCustomizer Admin Page', 'woocustomizer' ); ?>
						</a>
						<a href="<?php echo esc_url( 'https://storecustomizer.com/go/woocustomizer-settings/' ); ?>" target="_blank" class="wcz-notice-btn">
							<?php esc_html_e( 'Try out StoreCustomizer PRO Settings', 'woocustomizer' ); ?>
						</a>
					</div>
					<div class="wcz-notice-col">
						<h5><?php esc_html_e( 'Have you found a bug?', 'woocustomizer' ); ?><br /><?php esc_html_e( 'Something not working as expected?', 'woocustomizer' ); ?></h5>
						<div class="wcz-notice-cont">
							<p><?php esc_html_e( 'Please take it easy on us if something isn\'t working, get in contact and I\'ll try to fix it right away for you.', 'woocustomizer' ); ?></p>
							<p>
								<?php
								/* translators: 1: 'giving us a review'. */
								printf( esc_html__( '%1$s, am here to help, and fix any bugs that come up, manage support & development, and also adding the new features you want.', 'woocustomizer' ), wp_kses( '<a href="https://storecustomizer.com/go/about/" class="wcz-admin-notice-a" target="_blank">' . __( 'I, Zack', 'woocustomizer' ) . '</a>', array( 'a' => array( 'href' => array(), 'class' => array(), 'target' => array() ) ) ) ); ?>
							</p>
						</div>

						<a href="<?php echo esc_url( 'https://storecustomizer.com/go/contact/' ); ?>" target="_blank" class="wcz-notice-btn">
							<?php esc_html_e( 'Report a Bug / Request a Feature', 'woocustomizer' ); ?>
						</a>
					</div>
					<div class="wcz-notice-col last">
						<h5><?php esc_html_e( 'Like StoreCustomizer? Give it 5 stars :)', 'woocustomizer' ); ?></h5>
						<div class="wcz-notice-cont">
							<p><?php esc_html_e( 'If the plugin works well for you or our support was good... giving StoreCustomizer a nice review would really help with motivation & giving other users trust in my work.', 'woocustomizer' ); ?></p>
                        </div>
                        <div class="wcz-notice-rating-btns">
                            <div class="wcz-notice-rating-btn wcz-notice-rating-click">
                                <?php esc_html_e( 'Ok, I\'ll rate StoreCustomizer!', 'woocustomizer' ); ?>
                            </div>
                            <div class="wcz-notice-rating-options">
                                <p><?php esc_html_e( 'If there\'s something you\'re not happy with, please let me try and help you fix it.', 'woocustomizer' ); ?></p>
                                <a href="<?php echo esc_url( 'https://storecustomizer.com/go/contact/' ); ?>" target="_blank" class="wcz-notice-rating-btn"><?php esc_html_e( 'I\'m not happy, Please help!', 'woocustomizer' ); ?></a>
                                <a href="<?php echo esc_url( 'https://storecustomizer.com/go/rating/' ); ?>" target="_blank" class="wcz-notice-rating-btn"><?php esc_html_e( 'I\'ll give you a good rating!', 'woocustomizer' ); ?></a>
                            </div>
                        </div>
					</div>
				</div>

				<a href="<?php echo esc_url( admin_url( $wczpage . 'wcz_feedback_notice_ignore' ) ); ?>" class="wcz-notice-close"><?php esc_html_e( 'Dismiss Notice', 'woocustomizer' ); ?></a>
			</div><?php
		endif;
	}
     ...
     * Admin notice to ask for feedback
     */
    public function wcz_pricechange_notice() {
        global $pagenow;
        global $current_user;
        $wcz_user_id = $current_user->ID;
        $wczpage = isset( $_GET['page'] ) ? $pagenow . '?page=' . $_GET['page'].'&' : $pagenow.'?';

        if ( current_user_can( 'manage_options' ) && !get_user_meta( $wcz_user_id, 'wcz_pricechange_notice_dismiss' ) && 'admin.php?page=wcz_settings&' == $wczpage ) : ?>
            <div class="notice notice-info wcz-admin-notice wcz-price-notice">
                <h4>
                    <?php esc_html_e( 'A heads up on StoreCustomizer Pro price change!', 'woocustomizer' ); ?>
                </h4>
                <p>
                    <?php
                    /* translators: 1: 'WooCommerce Custom Product Badges feature', 2: 'purchase StoreCustomizer Pro now'. */
                    printf( esc_html__( 'With the new upcoming features, StoreCustomizer Pro\'s price will increase slightly when the %1$s gets added... if you want to pay only $29... then %2$s. Oh... and use the coupon code "LASTCHANCE" and we\'ll give you a further 20&#x25; OFF !', 'woocustomizer' ), wp_kses( '<a href="' . admin_url( 'admin.php?page=wcz_settings&tab=wcz_product_badges' ) . '" class="wcz-admin-notice-a">' . __( 'WooCommerce Custom Product Badges feature', 'woocustomizer' ) . '</a>', array( 'a' => array( 'href' => array(), 'class' => array() ) ) ), wp_kses( '<a href="' . admin_url( 'admin.php?billing_cycle=annual&page=wcz_settings-pricing' ) . '" class="wcz-admin-notice-a">' . __( 'purchase StoreCustomizer Pro now', 'woocustomizer' ) . '</a>', array( 'a' => array( 'href' => array(), 'class' => array() ) ) ) ); ?>
                </p>
                <a href="<?php echo esc_url( 'https://storecustomizer.com/#anchor-premium' ); ?>" target="_blank"><?php esc_html_e( 'See All features included in StoreCustomizer Pro', 'woocustomizer' ); ?></a> &nbsp; | &nbsp; <a href="<?php echo esc_url( admin_url( $wczpage . 'wcz_pricechange_notice_dismiss' ) ); ?>"><?php esc_html_e( 'Dismiss This Notice', 'woocustomizer' ); ?></a>
                <a href="<?php echo esc_url( admin_url( $wczpage . 'wcz_pricechange_notice_dismiss' ) ); ?>" class="wcz-notice-close"><?php esc_html_e( 'Dismiss Notice', 'woocustomizer' ); ?></a>
            </div><?php
        endif;
    }

    public function wcz_pricechange_notice_ignore() {
        global $current_user;
        $wcz_user_id = $current_user->ID;
    
        if ( isset( $_GET['wcz_pricechange_notice_dismiss'] ) ) {
            update_user_meta( $wcz_user_id, 'wcz_pricechange_notice_dismiss', true );
        }
    }
