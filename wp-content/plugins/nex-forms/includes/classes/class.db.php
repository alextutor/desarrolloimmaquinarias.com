...
/* INSERT */
		...
		public function get_c_logic_ui($form_id=0){
			...
			//else
				$c_logic = json_encode($_POST['conditional_logic_array']);
		...
		foreach($rules as $rule)
			...
			if($rule)
				...
				foreach($get_actions as $set_action)
					...
					$get_action_to =  explode('[end_to_value]',$action_to[1]);


					//$action_skip_to_step =  explode('[skip_to_step]',$set_action);
					//$get_action_skip_to_step =  explode('[end_skip_to_step]',$action_skip_to_step[1]);
					...
					//if($get_action_to_take=='skip_to')
					//	$action_field_type = 'step_skip';
