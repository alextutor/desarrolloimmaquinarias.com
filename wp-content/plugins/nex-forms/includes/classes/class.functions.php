					...
					foreach($rule->conditions as $condition)
						...
						if($con_field_type)
							...
							if($get_the_condition=='contains' || $get_the_condition=='not_contains')
								...
								if($con_field_type=='select')
									{
									$output .= $set_bool.'strstr(nf_str_to_lower(jQuery(\'#nf_form_'.$unigue_form_Id.' #'.$con_field_id.'\').find(\'select option:selected\').val()), '.$set_the_value.')';
									}
								else if($con_field_type=='textarea')
									{
									$output .= $set_bool.'strstr(nf_str_to_lower(jQuery(\'#nf_form_'.$unigue_form_Id.' #'.$con_field_id.'\').find(\'textarea\').val()), '.$set_the_value.')';
									}
								else if($con_field_id=='hidden_field')
									{
									$output .= $set_bool.'strstr(nf_str_to_lower(jQuery(\'#nf_form_'.$unigue_form_Id.'\').find(\'input[name="'.$con_field_name.'"][type="hidden"]\').val()), '.$set_the_value.')';
									}
								else
									{
									$output .= $set_bool.'strstr(nf_str_to_lower(jQuery(\'#nf_form_'.$unigue_form_Id.' #'.$con_field_id.'\').find(\'input[type="'.$con_field_type.'"]'.$add_string.'\').val()), '.$set_the_value.')';
									}
							...
							if($reverse_action=='true' || !$reverse_action)
								...
								if($action_field_type=='step')
									...
									if($get_action_to_take=='show')
										...
										if(obj)
											{
											//obj.closest(".step").find(".nex-step").removeAttr("data-skip-to");
											}';
										}
									}
								else
					...
					jQuery(document).on(\'change\', \'#nex-forms input , #nex-forms select, #nex-forms textarea\',
						function()
							...
							if(jQuery(this).is(":checkbox")){
							  setTimeout(function(){ run_nf_conditional_logic'.$function_post_fix.'(the_obj); }, 300);
							}
							else
