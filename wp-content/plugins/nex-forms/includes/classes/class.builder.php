...
if(!class_exists('NEXForms_Builder7')){
	class NEXForms_Builder7{
		...
		public function __construct(){
			...
			$form_id = isset($_REQUEST['open_form']) ? $_REQUEST['open_form'] : '1';
			$tut_id = isset($_REQUEST['tut']) ? $_REQUEST['tut'] : '0';
		...
		public function get_c_logic_ui(){
			...
			$c_logic = $wpdb->get_var('SELECT conditional_logic_array FROM '.$wpdb->prefix.'wap_nex_forms WHERE Id='.$form_Id);
		...
		public function builder7_top_menu(){
					...
					$output .= '
						<div class="col-sm-12">
						  <div class="help_menu dropdown_menu aa_menu">
								...
								<li><a class="btn waves-effect waves-light  tutorial-menu"><span class="fa fa-graduation-cap"></span></a>
								...
								<li><a class="btn waves-effect waves-light tours-menu"><span class="fa fa-info-circle"></span></a>
									<ul class="aa_bg_sec aa_menu_2">
						...
						$output .= '<li class="back-to-dashboard" ><a class="btn waves-effect waves-light btn-dashboard" href="'.get_admin_url().'admin.php?page=nex-forms-dashboard" data-toggle="tooltip_bs" data-placement="bottom" title="'.__('Back to NEX-Forms Dashboard','nex-forms').'"><span class="fa fa-dashboard"></span></a></li>';
						
						$output .= '<li class="open-form-item"><a class="btn waves-effect waves-light saved-forms" href="#" data-toggle="tooltip_bs2" data-placement="bottom" title="'.__('Forms','nex-forms').'"><span class="fas fa-folder-open"></span></a>
											  ...
											  $output .= '</ul>';
						$output .= '<li><a class="btn waves-effect waves-light  create_new_form" data-toggle="tooltip_bs2" data-placement="bottom" title="'.__('Create New Form','nex-forms').'"><span class="fas fa-folder-plus"></span></a></li>';
				...
				$output .= '<div class="nav-content prime-menu">';
						...
						$output.= '<div class="top-menu-dropdown"><ul class="tabs_nf sec-menu aa_menu"> 
										...
										'.((!get_option('nf_activated') || $theme->Name=='NEX-Forms Demo') ? '<li><a href="https://1.envato.market/6vWmb" target="_blank" class="btn waves-effect waves-light upgrade_pro">BUY NEX-FORMS</a>' : '').'</li>
									  </ul>
									  </div>
...
//LABEL SETTINGS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////				
		...
		$bc_data_theme 					= ($bc_settings['0']['data_theme']) ? $bc_settings['0']['data_theme'] 									: 'light-green';
		...
		$bc_gutter 						= ($bc_settings['0']['bc_gutter']) ? $bc_settings['0']['bc_gutter'] 									: '20';
		$bc_folded 						= ($bc_settings['0']['bc_folded']) ? $bc_settings['0']['bc_folded'] 									: 'bc-unfolded';
		$bc_connected 					= ($bc_settings['0']['bc_connected']) ? $bc_settings['0']['bc_connected'] 								: 'bc-connected';
		$bc_style 						= ($bc_settings['0']['bc_style']) ? $bc_settings['0']['bc_style'] 										: 'bc-solid';
		$bc_css 						= ($bc_settings['0']['bc_css']) ? $bc_settings['0']['bc_css'] 											: '';
		$multi_step_transition_in		= ($bc_settings['0']['multi_step_transition_in']) ? $bc_settings['0']['multi_step_transition_in'] 		: 'fadeIn';
		$multi_step_transition_out		= ($bc_settings['0']['multi_step_transition_out']) ? $bc_settings['0']['multi_step_transition_out'] 	: 'fadeOut';
		$multi_step_back_disabled		= ($bc_settings['0']['multi_step_back_disabled']) ? $bc_settings['0']['multi_step_back_disabled'] 		: 'no';
		...
		$output .= '<div id="ms-overall-settings-panel" class="ms-settings row settings-section active">';
			...
			$output .= '<div class="field-setting col-xs-6 s-all">';	
								$output .= '<div role="group" class="btn-group disable_back" data-value="'.$multi_step_back_disabled.'">';
									...
									$output .= '<button class="btn btn-default waves-effect waves-light yes '.(($multi_step_back_disabled!='no') ? 'active' : '' ).'" type="button" title="'.__('Allow user to step back','nex-forms').'"><i class="fa fa-check"></i></button>';
									$output .= '<button class="btn btn-default waves-effect waves-light no '.(($multi_step_back_disabled=='no') ? 'active' : '' ).'" type="button" title="'.__('Prevents user from stepping back','nex-forms').'"><i class="fa fa-close"></i></button>';
								$output .= '</div>';
							$output .= '</div>';
		...
		$output .= '<div id="ms-css-settings-panel" class="ms-settings row settings-section" style="display:none;">';
							...
							$output .= '<div class="field-setting col-xs-4 s-all">';
								$output .= '<div role="group" class="btn-group bc_position" data-value="'.$bc_position.'">';
									...
									$output .= '<button class="btn btn-default waves-effect waves-light position_top '.(($bc_position=='top') ? 'active' : '' ).'" type="button" title="'.__('Add to Form Header','nex-forms').'"><i class="fa fa-window-maximize"></i></button>';
									$output .= '<button class="btn btn-default waves-effect waves-light position_bottom '.(($bc_position=='bottom') ? 'active' : '' ).'" type="button" title="'.__('Add to Form Footer','nex-forms').'"><i style="transform:rotate(180deg);" class="fa fa-window-maximize"></i></button>';
									//$output .= '<button class="btn btn-default waves-effect waves-light position_both '.(($bc_position=='both') ? 'active' : '' ).'" type="button" title="'.__('Display top and <br />bottom of the Form','nex-forms').'"><i class="fa fa-arrows-v"></i></button>';
								$output .= '</div>';
							$output .= '</div>';
							...
							*/
							...
							$output .= '<div class="field-setting col-xs-4 s-all">';
								$output .= '<div role="group" class="btn-group bc_folded" data-value="'.$bc_folded.'">';
									...
									$output .= '<button class="btn btn-default waves-effect waves-light folded '.(($bc_folded=='bc-folded') ? 'active' : '' ).'" type="button" title="'.__('Display Step title<br>only when active','nex-forms').'"><i class="fas fa-book"></i></button>';
									$output .= '<button class="btn btn-default waves-effect waves-light unfolded '.(($bc_folded=='bc-unfolded') ? 'active' : '' ).'" type="button" title="'.__('Always display Step title','nex-forms').'"><i class="fas fa-book-open"></i></button>';
								$output .= '</div>';
							$output .= '</div>';
							...
							$output .= '<div class="field-setting col-xs-4 s-all">';
								$output .= '<div role="group" class="btn-group bc_connected" data-value="'.$bc_connected.'">';
									...
									$output .= '<button class="btn btn-default waves-effect waves-light connected '.(($bc_connected=='bc-connected') ? 'active' : '' ).'" type="button" title="'.__('Connect items','nex-forms').'"><i class="fa fa-link"></i></button>';
									$output .= '<button class="btn btn-default waves-effect waves-light unconnected '.(($bc_connected=='bc-unconnected') ? 'active' : '' ).'" type="button" title="'.__('Disconnect items','nex-forms').'"><i class="fa fa-unlink"></i></button>';
								$output .= '</div>';
							$output .= '</div>';
	...
	/*** Background position ***/									
						...
						$add_timer				= ($bc_settings['0']['add_timer']) ? $bc_settings['0']['add_timer'] 						: 'no';
						$timer_add_to			= ($bc_settings['0']['timer_add_to']) ? $bc_settings['0']['timer_add_to'] 					: 'header';
						
						$timer_type				= ($bc_settings['0']['timer_type']) ? $bc_settings['0']['timer_type'] 						: 'overall';
						
						$timer_size				= ($bc_settings['0']['timer_size']) ? $bc_settings['0']['timer_size'] 						: 'small';
						
						$timer_animation		= ($bc_settings['0']['timer_animation']) ? $bc_settings['0']['timer_animation'] 			: 'smooth';
						
						
						
						$timer_position			= ($bc_settings['0']['timer_position']) ? $bc_settings['0']['timer_position'] 				: 'timer_inline';
						$timer_align			= ($bc_settings['0']['timer_align']) ? $bc_settings['0']['timer_align'] 					: 'timer_right';
						
						$enabled_units			= ($bc_settings['0']['enabled_units']) ? $bc_settings['0']['enabled_units'] 				: 'minutes,seconds';
						$timer_hours			= ($bc_settings['0']['timer_hours']) ? $bc_settings['0']['timer_hours'] 					: 0;
						$timer_minutes			= ($bc_settings['0']['timer_minutes']) ? $bc_settings['0']['timer_minutes'] 				: 0;
						$timer_seconds			= ($bc_settings['0']['timer_seconds']) ? $bc_settings['0']['timer_seconds'] 				: 30;
						
						
						$timer_hours_label			= ($bc_settings['0']['timer_hours_label']) ? $bc_settings['0']['timer_hours_label'] 				: '';
						$timer_minutes_label		= ($bc_settings['0']['timer_minutes_label']) ? $bc_settings['0']['timer_minutes_label'] 			: '';
						$timer_seconds_label		= ($bc_settings['0']['timer_seconds_label']) ? $bc_settings['0']['timer_seconds_label'] 			: '';
						
						
						$timer_hours_color			= ($bc_settings['0']['timer_hours_color']) ? $bc_settings['0']['timer_hours_color'] 				: '#2979FF';
						$timer_minutes_color		= ($bc_settings['0']['timer_minutes_color']) ? $bc_settings['0']['timer_minutes_color'] 			: '#00bcd4';
						$timer_seconds_color		= ($bc_settings['0']['timer_seconds_color']) ? $bc_settings['0']['timer_seconds_color'] 			: '#40C4FF';
						
						
						$timer_direction		= ($bc_settings['0']['timer_direction']) ? $bc_settings['0']['timer_direction'] 			: 'clockwise';
						
						$timer_text_color				= ($bc_settings['0']['timer_text_color']) ? $bc_settings['0']['timer_text_color'] 						: '#888';
						$timer_inner_circle_color		= ($bc_settings['0']['timer_inner_circle_color']) ? $bc_settings['0']['timer_inner_circle_color'] 		: '#aaa';
						
						$timer_bg_width			= ($bc_settings['0']['timer_bg_width']) ? $bc_settings['0']['timer_bg_width'] 	: '0.1';
						$timer_fg_width			= ($bc_settings['0']['timer_fg_width']) ? $bc_settings['0']['timer_fg_width'] 	: '0.05';
						
						$timer_start			= ($bc_settings['0']['timer_start']) ? $bc_settings['0']['timer_start'] 	: '1';
						$timer_end				= ($bc_settings['0']['timer_end']) ? $bc_settings['0']['timer_end'] 	: '0';
						...
						$output .= '<div id="ms-timer-settings-panel" class="ms-settings row settings-section" style="display:none;">';
								...
								$output .= '<div class="field-setting col-xs-3 s-all" >';	
									$output .= '<div role="group" class="btn-group add-timer" data-value="'.$add_timer.'">';
										...
										$output .= '<button class="btn btn-default waves-effect waves-light yes '.(($add_timer=='yes') ? 'active' : '' ).'" type="button" title="'.__('Adds a timer to the form','nex-forms').'"><i class="fa fa-check"></i></button>';
										$output .= '<button class="btn btn-default waves-effect waves-light no '.(($add_timer=='no') ? 'active' : '' ).'" type="button" title="'.__('No time limit','nex-forms').'"><i class="fa fa-close"></i></button>';
									$output .= '</div>';
								$output .= '</div>';
								...
								$output .= '<div class="field-setting col-xs-3 s-all">';
									$output .= '<div role="group" class="btn-group timer-add-to" data-value="'.$timer_add_to.'">';
												$output .= '<small>'.__('Add To','nex-forms').'</small>';
												$output .= '<button class="btn btn-default waves-effect waves-light header 		'.(($timer_add_to=='header') ? 'active' : '' ).' " 		type="button" title="'.__('Add to Form Header','nex-forms').'"><i class="fa fa-window-maximize"></i></button>';
												$output .= '<button class="btn btn-default waves-effect waves-light footer 		'.(($timer_add_to=='footer') ? 'active' : '' ).'" 	type="button" title="'.__('Add to Form Footer','nex-forms').'"><i style="transform:rotate(180deg);" class="fa fa-window-maximize"></i></button>';
											$output .= '</div>';
								$output .= '</div>';	
								...
								$output .= '<div class="field-setting col-xs-3 s-all">';
									$output .= '<div role="group" class="btn-group timer-position" data-value="'.$timer_position.'">';
												$output .= '<small>'.__('Position','nex-forms').'</small>';
												$output .= '<button class="btn btn-default waves-effect waves-light timer_inline	'.(($timer_position=='timer_inline') ? 'active' : '' ).' " 	type="button" title="'.__('Inline with Breadcrumb','nex-forms').'"><i class="fa fa-arrows-h"></i></button>';
												$output .= '<button class="btn btn-default waves-effect waves-light timer_top 		'.(($timer_position=='timer_top') ? 'active' : '' ).'" 			type="button" title="'.__('Top of Breadcrumb','nex-forms').'"><i class="fa fa-arrow-up"></i></button>';
												$output .= '<button class="btn btn-default waves-effect waves-light timer_bottom 	'.(($timer_position=='timer_bottom') ? 'active' : '' ).'" 	type="button" title="'.__('Bottom of Breadcrumb','nex-forms').'"><i class="fa fa-arrow-down"></i></button>';
											$output .= '</div>';
								$output .= '</div>';	
								$output .= '<div class="field-setting col-xs-3 s-all">';
									$output .= '<div role="group" class="btn-group timer-align" data-value="'.$timer_align.'">';
												$output .= '<small>'.__('Alignment','nex-forms').'</small>';
												$output .= '<button class="btn btn-default waves-effect waves-light timer_left 		'.(($timer_align=='timer_left') ? 'active' : '' ).' " 		type="button" title="'.__('Left','nex-forms').'"><i class="fa fa-align-left"></i></button>';
												$output .= '<button class="btn btn-default waves-effect waves-light timer_center 	'.(($timer_align=='timer_center') ? 'active' : '' ).'" 	type="button" title="'.__('Center','nex-forms').'"><i class="fa fa-align-center"></i></button>';
												$output .= '<button class="btn btn-default waves-effect waves-light timer_right 	'.(($timer_align=='timer_right') ? 'active' : '' ).'" 		type="button" title="'.__('Right','nex-forms').'"><i class="fa fa-align-right"></i></button>';
											$output .= '</div>';
								$output .= '</div>';			
								...
								$output .= '<div class="field-setting col-xs-5 s-all" >';	
									$output .= '<div role="group" class="btn-group timer-type" data-value="'.$timer_type.'">';
										...
										$output .= '<button class="btn btn-default text-btn waves-effect waves-light overall '.(($timer_type=='overall') ? 'active' : '' ).'" type="button" title="'.__('Adds a time limit to the form overall','nex-forms').'">'.__('Overall Form','nex-forms').'</button>';
										$output .= '<button class="btn btn-default text-btn waves-effect waves-light per_step '.(($timer_type=='per_step') ? 'active' : '' ).'" type="button" title="'.__('Adds a time limit each step','nex-forms').'">'.__('Individual Steps','nex-forms').'</button>';
									$output .= '</div>';
								$output .= '</div>';
								...
								$output .= '<div class="field-setting col-xs-4 s-all">';
										...
										$output .= '<button class="btn btn-default waves-effect waves-light btn-sm '.((in_array('hours',$enabled_units_array)) ? 'active' : '').' units_0" data-val="hours" type="button"><i class="btn-tx">Hrs</i></button>';
										$output .= '<button class="btn btn-default waves-effect waves-light btn-sm '.((in_array('minutes',$enabled_units_array)) ? 'active' : '').' units_1" data-val="minutes" type="button"><i class="btn-tx">Mns</i></button>';
										$output .= '<button class="btn btn-default waves-effect waves-light btn-sm '.((in_array('seconds',$enabled_units_array)) ? 'active' : '').' units_2" data-val="seconds" type="button"><i class="btn-tx">Scs</i></button>';
								...
								$output .= '<div class="field-setting col-xs-4 s-all" >';	
									$output .= '<div role="group" class="btn-group timer-size" data-value="'.$timer_size.'">';
										$output .= '<small>'.__('Size','nex-forms').'</small>';
										$output .= '<button class="btn btn-default btn-sm waves-effect waves-light extra-small 	'.(($timer_size=='extra-small') ? 'active' : '' ).'" type="button" title="'.__('Extra Small','nex-forms').'"><i class="btn-tx">'.__('Xs','nex-forms').'</i></button>';
										$output .= '<button class="btn btn-default btn-sm waves-effect waves-light small 			'.(($timer_size=='small') ? 'active' : '' ).'" type="button" title="'.__('Small','nex-forms').'"><i class="btn-tx">'.__('S','nex-forms').'</button></i>';
										$output .= '<button class="btn btn-default btn-sm waves-effect waves-light normal 			'.(($timer_size=='normal') ? 'active' : '' ).'" type="button" title="'.__('Small Medium','nex-forms').'"><i class="btn-tx">'.__('Sm','nex-forms').'</i></button>';
										$output .= '<button class="btn btn-default btn-sm waves-effect waves-light large 			'.(($timer_size=='large') ? 'active' : '' ).'" type="button" title="'.__('Large','nex-forms').'"><i class="btn-tx">'.__('L','nex-forms').'</button></i>';
										$output .= '<button class="btn btn-default btn-sm waves-effect waves-light extra-large 	'.(($timer_size=='extra-large') ? 'active' : '' ).'" type="button" title="'.__('Extra Large','nex-forms').'"><i class="btn-tx">'.__('Xl','nex-forms').'</i></button>';
										//$output .= '<button class="btn btn-default waves-effect waves-light super-size 		'.(($timer_size=='super-size') ? 'active' : '' ).'" type="button" title="'.__('Super Size','nex-forms').'">'.__('Sz','nex-forms').'</button>';
									$output .= '</div>';
								$output .= '</div>'; 
								...
								$output .= '<div class="field-setting col-xs-3 s-all" >';	
									$output .= '<div role="group" class="btn-group timer-animation" data-value="'.$timer_animation.'">';
										...
										$output .= '<button class="btn btn-default text-btn waves-effect waves-light smooth '.(($timer_animation=='smooth') ? 'active' : '' ).'" type="button" title="'.__('Smooth Transition','nex-forms').'">Smooth</button>';
										$output .= '<button class="btn btn-default text-btn waves-effect waves-light ticks '.(($timer_animation=='ticks') ? 'active' : '' ).'" type="button" title="'.__('Ticking Transition','nex-forms').'">Ticks</button>';
									$output .= '</div>';
								$output .= '</div>';
								$output .= '<div class="field-setting col-xs-1 s-all" >';
								$output .= '</div>';
								...
								$output .= '<div class="field-setting col-xs-3 s-all" >';	
									$output .= '<div role="group" class="btn-group timer-direction" data-value="'.$timer_direction.'">';
										...
										$output .= '<button class="btn btn-default waves-effect waves-light anti-clockwise '.(($timer_direction=='anti-clockwise') ? 'active' : '' ).'" type="button" title="'.__('Clockwise','nex-forms').'"><i style="transform:rotate(270deg);" class="fa fa-redo"></i></button>';
										$output .= '<button class="btn btn-default waves-effect waves-light clockwise '.(($timer_direction=='clockwise') ? 'active' : '' ).'" type="button" title="'.__('Anti-Clockwise','nex-forms').'"><i style="transform:rotate(90deg);" class="fa fa-undo"></i></button>';
							...
							*/
							...
							$output .= '<div class="field-setting col-xs-4 s-all msg-editor-only">';
								$output .= '<div role="group" class="btn-group msg_hide_form" data-value="'.$msg_hide_form.'">';
									...
									$output .= '<button class="btn btn-default waves-effect waves-light yes '.(($msg_hide_form=='yes') ? 'active' : '' ).'" type="button" title="'.__('Hide Form','nex-forms').'"><i class="fa fa-check"></i></button>';
									$output .= '<button class="btn btn-default waves-effect waves-light no '.(($msg_hide_form=='no') ? 'active' : '' ).'" type="button" title="'.__('Dont Hide Form','nex-forms').'"><i class="fa fa-close"></i></button>';
									//$output .= '<button class="btn btn-default waves-effect waves-light position_both '.(($bc_position=='both') ? 'active' : '' ).'" type="button" title="'.__('Display top and <br />bottom of the Form','nex-forms').'"><i class="fa fa-arrows-v"></i></button>';
								$output .= '</div>';
							$output .= '</div>';
							...
							$output .= '<div class="field-setting col-xs-4 s-all msg-editor-only">';
								$output .= '<div role="group" class="btn-group msg_position" data-value="'.$msg_position.'">';
									...
									$output .= '<button class="btn btn-default waves-effect waves-light top '.(($msg_position=='top') ? 'active' : '' ).'" type="button" title="'.__('Show above the Form','nex-forms').'"><i class="fa fa-window-maximize"></i></button>';
									$output .= '<button class="btn btn-default waves-effect waves-light bottom '.(($msg_position=='bottom') ? 'active' : '' ).'" type="button" title="'.__('Show below the Form','nex-forms').'"><i style="transform:rotate(180deg);" class="fa fa-window-maximize"></i></button>';
									//$output .= '<button class="btn btn-default waves-effect waves-light position_both '.(($bc_position=='both') ? 'active' : '' ).'" type="button" title="'.__('Display top and <br />bottom of the Form','nex-forms').'"><i class="fa fa-arrows-v"></i></button>';
								$output .= '</div>';
							$output .= '</div>';
							...
							$output .= '<div class="field-setting col-xs-4 s-all msg-editor-only">';
								$output .= '<div role="group" class="btn-group msg_placement" data-value="'.$msg_placement.'">';
									...
									$output .= '<button class="btn btn-default waves-effect waves-light outside '.(($msg_placement=='outside') ? 'active' : '' ).'" type="button" title="'.__('Show Outside the<br />Form Wrapper','nex-forms').'"><i class="fa fa-sign-out-alt"></i></button>';
									$output .= '<button class="btn btn-default waves-effect waves-light inside '.(($msg_placement=='inside') ? 'active' : '' ).'" type="button" title="'.__('Show Inside the<br />Form Wrapper','nex-forms').'"><i class="fa fa-sign-in-alt"></i></button>';
									//$output .= '<button class="btn btn-default waves-effect waves-light position_both '.(($bc_position=='both') ? 'active' : '' ).'" type="button" title="'.__('Display top and <br />bottom of the Form','nex-forms').'"><i class="fa fa-arrows-v"></i></button>';
								$output .= '</div>';
							$output .= '</div>';
...
/*** Date Format Placeholder ***/
							...
							$output .= '<div class="field-setting col-xs-4 s-time s-date set-popup-keep-open">';	
										$output .= '<div role="group" class="btn-group  popup-keep-open">';
														...
														$output .= '<button class="btn btn-default waves-effect waves-light btn-sm yes" title="Disable the selection<br />of Past Dates" type="button"><i class="fa fa-check"></i></button>';
														$output .= '<button class="btn btn-default waves-effect waves-light btn-sm no active" title="Enable the selection<br />of Past Dates" type="button"><i class="fa fa-remove"></i></button>';
													$output .= '</div>';
									$output .= '</div>';
...
/*** Date Format Language ***/	
	...
	/*** Input Styling ***/		
							...
							$output .= '<div class="field-setting no-batch-edit col-xs-12 s-text s-select s-spinner s-submit s-headings s-tags s-math">';	
	...
	/*** Heading Options ***/
			...
			/*** heading alignment ***/		
							...
							$output .= '<div class="field-setting col-xs-6 s-paragraph s-headings s-html s-divider s-icon s-math">';
								$output .= '<small>'.__('Field Wrapper Margins','nex-forms').'</small>';
			...
			/**** GRID SETTINGS ****/
										...
										$output .= '<div class="field-setting col-xs-12 s-grid">';	
											$output .= '<div role="group" class="btn-group setting-responsive-grid">';
												...
												$output .= '<button class="btn btn-default waves-effect waves-light set-xs" type="button" title="'.__('Grid stays inline no matter<br />the device max width','nex-forms').'"><i class="fa fa-mobile"></i></button>';
												$output .= '<button class="btn btn-default waves-effect waves-light set-sm" type="button" title="'.__('If device max-width <= 480px<br />Break grids','nex-forms').'"><i class="fa fa-tablet"></i></button>';
												$output .= '<button class="btn btn-default waves-effect waves-light set-md" type="button" title="'.__('If device max-width <= 768px<br />Break grids').'"><i class="fa fa-laptop"></i></button>';
												$output .= '<button class="btn btn-default waves-effect waves-light set-lg" type="button" title="'.__('If device max-width <= 1024px<br />Break grids').'"><i class="fa fa-desktop"></i></button>';
...
//MATH SETTINGS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////				
							$output .= '<div class="math-settings settings-section row">';
								$output .= '<div class="field-setting no-batch-edit col-xs-6 s-headings s-math s-paragraph s-html">';
	/*** Input Placeholder ***/	;
	/*** Input Name ***/
	...
	/*** Input ID ***/			
								...
								$output .= '<div class="field-setting no-batch-edit col-xs-12 s-headings s-math s-paragraph s-html">';
		...
		public function print_form_canvas(){
			...
			$output .= '<div class="form-canvas-area form-editor-view '.$set_theme_shade.'" data-sec-pre-class=".inner-form-canvas">';
				...
				$output .= '<div class="preview-tools">';
					$output .= '<span class="expand_fullscreen"><a class="btn waves-effect waves-light btn-fullscreen fc-options" href="#" data-toggle="tooltip_bs" data-placement="bottom" title="'.__('Enter Full Screen Mode','nex-forms').'"><span class="fa fas fa-expand-arrows-alt"></span></a></span>';
					$output .= '<span class="colapse_fullscreen" style="display:none;"><a class="btn waves-effect waves-light btn-wordpress fc-options active" href="#" data-toggle="tooltip_bs" data-html="true" data-placement="bottom" title="'.__('Exit Fullscreen Mode<br>Show WordPress Menus','nex-forms').'"><span class="fas fa-compress-arrows-alt"></span></a></span>';
			...
			//SET PREFERENCES
							...
							$output .= '<div class="tool-spacer grid-selection"><span class="tool-section-title" title="Selection Fields" data-toggle="" data-placement="top">'.__('Grid System','nex-forms').'</span>';		
...
tiff</div>
																	  ...
																	  <div class="row step-settings">
																		<div class="col-xs-10">
																		  <div class="row">
																			...
																			<div class="col-xs-5"><small>Description</small>
				...
				$output .= '<div class="tool-section  muli-step-selection" >';
						...
						$output .= '<ul class="multi-step-stepping">';
							...
							$output .= wp_unslash($ms_settings['0']['multi_step_stepping']);
				...
				$output .= '<div class="width_bar" style="display:none;">';
					$output .= '<div class="width_input" style="display:none;">';
						...
						$msg_width_percentage 	= ($form_settings['0']['msg_width_percentage']) ? $form_settings['0']['msg_width_percentage'] : '100';
						$msg_width_pixels	 	= ($form_settings['0']['msg_width_pixels']) ? $form_settings['0']['msg_width_pixels'] : '950';
						$msg_width_unit	 		= ($form_settings['0']['msg_width_unit']) ? $form_settings['0']['msg_width_unit'] : '%';
				...
				$output .= '</div>';
					...
					$bc_type 				= ($bc_settings['0']['breadcrumb_type']) ? $bc_settings['0']['breadcrumb_type'] 	: 'pilled';
					$bc_text_pos 			= ($bc_settings['0']['text_pos']) ? $bc_settings['0']['text_pos'] 					: 'text-bottom';	
					$bc_data_theme 			= ($bc_settings['0']['data_theme']) ? $bc_settings['0']['data_theme'] 				: 'default';
					$bc_show_front_end 		= ($bc_settings['0']['show_front_end']) ? $bc_settings['0']['show_front_end'] 		: 'yes';	
					$bc_show_inside 		= ($bc_settings['0']['show_inside']) ? $bc_settings['0']['show_inside'] 			: 'no';	
					$scroll_to_top 			= ($bc_settings['0']['scroll_to_top']) ? $bc_settings['0']['scroll_to_top'] 		: 'yes';
					...
					$msg_width_percentage 	= ($bc_settings['0']['msg_width_percentage']) ? $bc_settings['0']['msg_width_percentage'] : '100';
					$msg_width_pixels	 	= ($bc_settings['0']['msg_width_pixels']) ? $bc_settings['0']['msg_width_pixels'] : '950';
					$msg_width_unit	 		= ($bc_settings['0']['msg_width_unit']) ? $bc_settings['0']['msg_width_unit'] 	: '%';
					...
					$bc_position	 		= ($bc_settings['0']['bc_position']) ? $bc_settings['0']['bc_position'] 			: 'top';
					$bc_gutter	 			= ($bc_settings['0']['bc_gutter'] || $bc_settings['0']['bc_gutter']=='0') ? $bc_settings['0']['bc_gutter'] : '20';
					$bc_folded 				= ($bc_settings['0']['bc_folded']) ? $bc_settings['0']['bc_folded'] 				: 'bc-unfolded';
					$bc_connected 			= ($bc_settings['0']['bc_connected']) ? $bc_settings['0']['bc_connected'] 			: 'bc-connected';
					$bc_style 				= ($bc_settings['0']['bc_style']) ? $bc_settings['0']['bc_style'] 					: 'bc-solid';
					$bc_css 				= ($bc_settings['0']['bc_css']) ? $bc_settings['0']['bc_css'] 						: '';
				...
				$breadcrumb = '<div class="bc-outer-container hide_on_load '.(($bc_show_front_end=='yes') ? '' : 'no-front-end').'" style="display:none;">
									...
									<div class="nf_ms_breadcrumb  bc-gutter-'.$bc_gutter.' bc-'.$bc_position.' '.$bc_style.' '.$bc_connected.' '.$bc_folded.' '.$bc_type.'" style="'.$bc_css.'">
										...
										'.$multistep_settings[0]['bc_position'].wp_unslash($bc_list).'
										</ul>
										<div class="nf_progressbar the_br" '.(($bc_type!='p_bar') ? 'style="display:none;"' : '').'  data-text-pos="'.$bc_text_pos.'" data-breadcrumb-type="'.$bc_type.'" data-theme="'.$bc_data_theme.'" data-show-front-end="'.$bc_show_front_end.'" data-show-inside="'.$bc_show_inside.'">
											<div class="nf_progressbar_percentage md-color-'.$bc_data_theme.'" style="width:5%;">
												<span>0%</span>
				...
				$timer_wrapper_css		= ($bc_settings['0']['timer_wrapper_css']) ? $bc_settings['0']['timer_wrapper_css'] 		: '';
...
$output .= '<div class="inner-form-canvas">';
				...
				$output .= '<div class="outer-container '.(($add_timer=='yes') ? 'has_time_limit timer_'.$timer_type : '' ).' '.$ms_back_disabled_class.' theme-'.(($set_form_theme=='m_design' || $set_form_theme=='neumorphism') ? $set_theme : '').'" style="display:none;">';
					...
					$output .= '<div id="droppable-container" class="active nex-forms-container panel-body inner-canvas-container  ui-nex-forms-container  '.$set_form_theme.'" data-form-theme="'.$set_form_theme.'" data-width-percentage="'.$form_width_percentage.'" data-width-pixels="'.$form_width_pixels.'" data-width-unit="'.$form_width_unit.'" style="'.$form_style.'">';
	...
	$output .= '</div>';
			...
			$output .= '
			'.(($theme->Name=='NEX-Forms Demo') ? '<!--<a href="http://codecanyon.net/item/nexforms-the-ultimate-wordpress-form-builder/7103891?license=regular&open_purchase_for_item_id=7103891&purchasable=source&ref=Basix" target="_blank"  style="padding:7px !important;" class="btn waves-effect waves-light upgrade_pro animated fadeInRight">BUY NEX-FORMS</a>-->' : '' ).'
							...
							echo  '<div class="col-xs-9 form-setup-column">';
								echo  '<div class="material_box">';
									...
									echo  '<div class="material_box_content">';
										...
										echo  '<div class="row">';
											echo  '<div class="integration_form_label" >'.__('Save Form Progress?<br><em>If enabled users can start a form and complete it at a later stage</em>','nex-forms').'</em></div>';
								...
								//echo  '<div class="material_box_content tour_hidden_fields_setup_2">';
									...
									echo  '<div class="row">';		
										...
										echo  '<div class="col-sm-2">';	
											echo '<button class="add_hidden_field tour_hidden_fields_setup_1"><span class="fa fa-plus"></span><span class="btn-tx">'.__('Add hidden Field','nex-forms').'</span></button></button>';
										echo  '</div>';
	...
	public function print_integration_setup(){
		...
		$set_code = true;
			...
			echo '<div class="form_attr_wrapper">';
				...
				echo '<div class="form_attr_left_menu aa_bg_sec aa_menu">';
				echo '<ul>';
					echo '<li class="active"><a class="show_paypal_setup"><span class="fa fa-paypal"></span> <span class="sidemenu_text">'.__('PayPal','nex-forms').'</span></a></li>';
					echo '<li><a class="show_pdf_setup"><span class="fa fa-file-pdf-o"></span> <span class="sidemenu_text">'.__('PDF Creator','nex-forms').'</span></a></li>';
				...
				echo '<div class="form_attr_settings_wrapper">';
					...
					echo '<div id="paypal_integration" class="integration">';
						...
						if ( function_exists('nf_not_found_notice_pp') || function_exists('run_nf_adv_paypal')){
							...
							echo '<div class="col-md-5 form-setup-column">';
								echo '<div class="material_box">';
							...
							echo '<div class="col-md-7 paypal-items-column">';
								echo '<div class="material_box">';
		...
		if($form_Id)
			...
			echo '<div class="">';
				...
				echo '<div class="row last">';
					...
					echo '<div class="integration_form_field no_input tour_pdf_setup_1" style="padding: 10px 10px 20px 10px;" >';
