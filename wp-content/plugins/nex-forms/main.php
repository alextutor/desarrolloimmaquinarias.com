...
function NEXForms5_main_menu(){
	...
	add_submenu_page( null, 'NF-Builder-View','Builder', NF_USER_LEVEL, 'nex-forms-builder', 'NEXForms_form_builder');
...
function NEXForms_my_menu_pages() {
    $hook = add_submenu_page(
            null, 'Page Title', 'Page Title', NF_USER_LEVEL, 'nex-forms-preview', function() {
    }
    );
	...
	$css_js_version = $config->plugin_version.'.14';

	   echo '<link rel="stylesheet" type="text/css" href="'.plugins_url( '/public/css/fa5/css/all.min.css?v='.$css_js_version,__FILE__).'" />';
	   echo '<link rel="stylesheet" type="text/css" href="'.plugins_url( '/public/css/fa5/css/v4-shims.min.css?v='.$css_js_version,__FILE__).'" />';
	   echo '<link rel="stylesheet" type="text/css" href="'.plugins_url( '/public/css/min/ui-bootstrap.css?v='.$css_js_version,__FILE__).'" />';
	   echo '<link rel="stylesheet" type="text/css" href="'.plugins_url( '/public/css/ui.css?v='.$css_js_version,__FILE__).'" />';
	   echo '<link rel="stylesheet" type="text/css" href="'.plugins_url( '/public/css/min/animate.css?v='.$css_js_version,__FILE__).'" />';
	   echo '<link rel="stylesheet" type="text/css" href="'.plugins_url( '/public/css/min/materialize-ui.css?v='.$css_js_version,__FILE__).'" />';

	   echo '<style type="text/css">#nex-forms .input-label:not(.image-choices-choice-text){margin-top:0 !important;}</style>';

	   echo '<script type="text/javascript" src="'.includes_url('js/jquery/jquery.min.js?v='.$css_js_version,__FILE__).'"></script>';
	   echo '<script type="text/javascript" src="'.includes_url('js/jquery/ui/core.min.js?v='.$css_js_version,__FILE__).'"></script>';
	   echo '<script type="text/javascript" src="'.includes_url('js/jquery/jquery.form.min.js?v='.$css_js_version,__FILE__).'"></script>';
	   echo '<script type="text/javascript" src="'.includes_url('js/jquery/ui/mouse.min.js?v='.$css_js_version,__FILE__).'"></script>';
	   ...
	   echo '<script type="text/javascript" src="'.plugins_url( '/public/js/min/bootstrap.min.js?v='.$css_js_version,__FILE__).'"></script>';
	   echo '<script type="text/javascript" src="'.plugins_url( '/libs/wow.min.js?v='.$css_js_version,__FILE__).'"></script>';
	   echo '<script type="text/javascript" src="'.plugins_url( '/public/js/min/jquery.raty-fa.js?v='.$css_js_version,__FILE__).'"></script>';
	   echo '<script type="text/javascript" src="'.plugins_url( '/public/js/min/jquery.timer.js?v='.$css_js_version,__FILE__).'"></script>';
	   echo '<script type="text/javascript" src="'.plugins_url( '/libs/materialize.min.js?v='.$css_js_version,__FILE__).'"></script>';
	   echo '<script type="text/javascript" src="'.plugins_url( '/public/js/nexf-onload-ui.js?v='.$css_js_version,__FILE__).'"></script>';
	   echo '<script type="text/javascript" src="'.plugins_url( '/libs/math.min.js?v='.$css_js_version,__FILE__).'"></script>';
	   echo '<script type="text/javascript" src="'.plugins_url( '/libs/moment.min.js?v='.$css_js_version,__FILE__).'"></script>';
	   echo '<script type="text/javascript" src="'.plugins_url( '/public/js/bootstrap-datetimepicker.js?v='.$css_js_version,__FILE__).'"></script>';
	   echo '<script type="text/javascript" src="'.plugins_url( '/public/js/min/jquery.bootstrap-touchspin.js?v='.$css_js_version,__FILE__).'"></script>';

	   $unigue_form_Id 	= rand(0,99999);

	$get_form 	= $wpdb->prepare('SELECT * FROM '.$wpdb->prefix.'wap_nex_forms WHERE Id = %d',filter_var($_REQUEST['form_Id'],FILTER_SANITIZE_NUMBER_INT));
	...
	if($form_attr->form_theme=='m_design' || $form_attr->form_theme=='neumorphism')
	 echo '<link rel="stylesheet" type="text/css" href="'.plugins_url( '/public/css/themes/'.$set_theme.'.css?v='.$css_js_version,__FILE__).'" />';
	...
	//ADD CONDITIONAL LOGIC SCRIPT
		...
		echo '<script type="text/javascript" name="js_con_1">'.$cl_script.'</script>';
	...
	echo '<script type="text/javascript">'.$custom_user_func_before_submit_js.'</script>';
	...
	$custom_user_func_after_submit_js .= '}';
	 ...
	 echo '<script type="text/javascript">'.$custom_user_func_after_submit_js.'</script>';
...
function NEXForms_test_page(){
		...
		/*global $wpdb;
		$result = $wpdb->query('DELETE FROM '.$wpdb->prefix.'wap_nex_forms_views');
		$result2 = $wpdb->query('DELETE FROM '.$wpdb->prefix.'wap_nex_forms_stats_interactions');
		*/
...
function NEXForms_ui_output( $atts , $echo='',$prefill_array='',$unigue_form_Id=''){
		...
		$output .= '<div id="nex-forms" data-loader="'.$loader_type.'"data-msg-hide-form="'.$msg_hide_form.'" data-msg-position="'.$msg_position.'" data-msg-placement="'.$msg_placement.'"  class="nex-forms '.$nf_functions->format_name($form_attr->title).' '.(($make_sticky=='yes' && !get_option('nf_activated')) ? 'do_nf_sticky' : '' ).' '.(($make_sticky=='yes') ? 'is-nf-sticky-form' : '').'">';
			...
			$output .= '<div class="set_form_theme theme-'.$set_theme.'">';
				...
				/*if($form_attr->on_screen_confirmation_message_admin)
					{
					$output .= '<div class="nex_success_message2p0" style="display:none;">';
						$output .= '<div class="nex_success_message_container nex_success_message" style="'.$msg_css_style.'">';
							$output .= wp_unslash($form_attr->on_screen_confirmation_message);
						$output .= '</div>';
					$output .= '</div>';
					}
				else
					{*/
						...
						$osmsgv2 .= '<div class="nex_success_message msg_'.$msg_position.' msgv2 animated" style="display:none;">';
							$osmsgv2 .= '<div class="nex_success_message_container" style="'.$msg_css_style.'">';
								$osmsgv2 .= '<div class="msg_text">'.wp_unslash($form_attr->on_screen_confirmation_message).'</div>';
							$osmsgv2 .= '</div>';
						$osmsgv2 .= '</div>';
				...
				$output .= '<div data-timer-start="'.$timer_start.'" data-timer-end="'.$timer_end.'" class="inner-canvas-container ui-nex-forms-container '.(($add_timer=='yes') ? 'has_time_limit timer_'.$timer_type : '' ).' '.(($form_attr->form_theme) ? $form_attr->form_theme : 'bootstrap').'" id="ui-nex-forms-container"  >';
					...
					if(!$multistep_settings[0]['bc_converted'] && $multistep_settings[0]['show_inside']=='no' && $multistep_settings[0]['show_front_end']=='yes' && ($multistep_settings[0]['bc_position']=='top' || !$multistep_settings[0]['bc_position']) )
					...
					if($multistep_settings['0']['multi_step_total'])
						{
						if($multistep_settings[0]['bc_converted'])
							{
						...
						//if($multistep_settings[0]['bc_converted'] && $multistep_settings[0]['show_front_end']=='yes' && ($multistep_settings[0]['bc_position']=='top' || !$multistep_settings[0]['bc_position']))
							//{
					...
					if($multistep_settings['0']['multi_step_total'] && $multistep_settings[0]['bc_converted'])
						{
						$output .= '</div>';
						}
					...
					if($form_attr->is_paypal=='yes' && function_exists('nf_not_found_notice_ppp'))
						...
						$output .= '<div class="current_field_on_focus" style="display:none">1</div>';
							...
							if(!$multistep_settings['0']['multi_step_total'] && $multistep_settings[0]['show_inside']=='yes' && $multistep_settings[0]['show_front_end']=='yes' && ($multistep_settings[0]['bc_position']=='top' || !$multistep_settings[0]['bc_position']))
								$output .= '<div class="nf_step_breadcrumb">'.str_replace('\\','',$form_attr->multistep_html).'</div>';
							...
							$output .= 	'<form id="nf_form_'.$unigue_form_Id.'" class="'.$set_ajax.'" name="nex_form" action="'.$post_action.'" method="'.$post_method.'" enctype="multipart/form-data">';
								...
								$output .= '<input type="hidden" name="ip" value="'.$_SERVER['REMOTmE_ADDR'].'">';
								...
								$_SERVER['PAGE_URL'] 			= $_SERVER['REQUEST_URI'];
								...
								$output .= '<div style="clear:both;"></div>';
								if(!get_option('nf_activated'))
									$output .= '<small class="free_version">Powered by <a href="https://1.envato.market/zQ6de">NEX-Forms</a></small>';
					...
					//PRINT MULTI-STEP BREADCRUMB (If outside Form Wrapper and position is bottom)
						...
						if(!$multistep_settings[0]['bc_converted'] && $multistep_settings[0]['show_inside']=='no' && $multistep_settings[0]['show_front_end']=='yes' && $multistep_settings[0]['bc_position']=='bottom')
							$output .= '<div class="nf_step_breadcrumb">'.str_replace('\\','',$form_attr->multistep_html).'</div>';
						...
						if($multistep_settings['0']['multi_step_total'])
							...
							if($multistep_settings[0]['bc_converted'] && $multistep_settings[0]['show_front_end']=='yes' && $multistep_settings[0]['bc_position']=='bottom')
								$output .= '<div class="bc-outer-container"><div style="'.$set_bc_css .'" class="nf_ms_breadcrumb '.$ms_back_disabled_class.' bc-gutter-'.$bc_gutter.' bc-'.$bc_position.' '.$bc_style.' '.$bc_connected.' '.$bc_folded.' '.$bc_type.' '.(($bc_show_front_end=='yes') ? '' : 'no-front-end').'">'.str_replace('\\','',$form_attr->multistep_html).'</div></div>';
							}
	...
	if($script_config['inc-wow']=='1' || !$script_config['inc-wow'])
		...
		wp_add_inline_script('nex-forms-materialize.min', $extra_script);
...
function enqueue_nf_admin_public_styles($hook){
	if(is_admin())
			wp_enqueue_style('nex-forms-public-admin', plugins_url( '/admin/css/'.NF_PATH.'public.css',__FILE__),'','7.5.17');
}
...
function NEXForms_dashboard(){
	...
	$output .= '<div class="builder-footer">';
			...
			$output .= '
			'.(($theme->Name=='NEX-Forms Demo') ? '<a href="http://codecanyon.net/item/nexforms-the-ultimate-wordpress-form-builder/7103891?license=regular&open_purchase_for_item_id=7103891&purchasable=source&ref=Basix&add=demo_be" target="_blank" class="btn waves-effect waves-light upgrade_pro animated fadeInRight">BUY NEX-FORMS</a>' : '' ).'
	...
	if(!get_option('7103891'))
		...
		$response = array('body'=>'{"ver":"true","client_info":{"Id":"1111111","item_code":"7103891","plugin":null,"publish":"0","added":"0000-00-00 00:00","last_update":"2020-06-05 12:12:12","purchase_code":"11111111-1111-1111-1111-111111111111","date_puchased":"2020-01-01T00:00:00","license_type":"Regular License","envato_user_name":"username_here","email_address":"email@nowhere.com","for_site":"' . get_site_url() . '","unique_key":"11111111","expiration_date":"1861923661","source":"wordpress.org","flagged":""}}');
		if(!get_option('7103891'))
...
function nf_prefix_register_resources(){
	...
	$css_js_version = $config->plugin_version.'.1';
...
/* UI SCRIPT REGISTRATION */
	if($script_config['inc-bootstrap']=='1' || !$script_config['inc-bootstrap'])
		wp_register_script('nex-forms-bootstrap.min',  plugins_url( '/public/js/min/bootstrap.min.js',__FILE__),'',$css_js_version);
	if($script_config['inc-wow']=='1' || !$script_config['inc-wow'])
